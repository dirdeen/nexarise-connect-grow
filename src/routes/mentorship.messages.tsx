...
function MessagesPage() {
  ...
  const conversation = conversationId ? findConversation(conversationId) : undefined;
  const [message, setMessage] = useState("");
  const [status, setStatus] = useState("");
  const visibleMessages = useMemo(() => conversation?.messages ?? [], [conversation]);

  function handleAttach() {
    setStatus("Attachment storage for mentorship messages is not connected yet.");
  }
  ...
  function handleSend() {
    ...
    setStatus("Mentorship message storage is not connected yet.");
  }
          ...
          <section className="rounded-2xl border border-border bg-card shadow-card">
            {!conversation ? (
              <div className="p-10 text-center text-sm text-muted-foreground">
                Select a conversation when mentorship messaging is available.
              </div>
            ) : (
              <>
